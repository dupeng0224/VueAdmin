...
export default {
  ...
  computed: {
    ...
    // 底部布局真实配置项定义
    southOptions () {
      const resizable = this.south.resizable ? createResizableOptions('south', 'n', this.south.resizable, this) : false
      return this.south
        ? {
          height: '40px',
          ...defaultOptions,
...
.xdh-layout {
  ...
  &-split__north {
    ...
    i {
      width: 50px;
      height: 100%;
      color: $--widgets-layout-split-color;
      background: $--widgets-layout-split-background-color;
      cursor: pointer;

      &:hover {
        background: $--widgets-layout-split-background-color-hover;
        color: $--widgets-layout-split-color-hover;
      }
    }
  }
